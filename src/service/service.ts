...
export async function addExpense(data: IAddExpenseForm) {
  ...
  export async function getExpenses(snapshot: QuerySnapshot): Promise<IExpense[]> {
    try {
      if (!snapshot.empty) {
        const data = snapshot.docs.map((doc) => {
          const data = doc.data();
          const dateDisplay = format(data.date.toDate(), 'dd MMM yy'); // Output: "08 Oct 24"
          return {
            ...data,
            id: doc.id,
            date: dateDisplay, // Converts Timestamp to Date
          };
        });
